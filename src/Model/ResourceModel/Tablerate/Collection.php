...
use Klikealo\Carrier\Model\ResourceModel\Tablerate\Import;
use Klikealo\Carrier\Model\ResourceModel\Tablerate\RateQuery;
use Klikealo\Carrier\Model\ResourceModel\Tablerate\RateQueryFactory;
...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function __construct(
        ...
        \Klikealo\Carrier\Model\Tablerate\Tablerate $carrierTablerate,
        \Klikealo\Carrier\Model\CarrierFactory $carrierFactory,
        \Klikealo\Carrier\Model\ResourceModel\Carrier\CollectionFactory $carrierCollectionFactory,
    ...
    public function getRate(\Magento\Quote\Model\Quote\Address\RateRequest $request)
    {
        ...
        $result = $connection->fetchRow($select, $bindings);/*var_dump($bindings);*/
     ...
     * @return void
     */
    private function importData(array $fields, array $values, int $carrierId)
    {
        $connection = $this->getConnection();
        $connection->beginTransaction();
        ...
        try {
            if (count($fields) && count($values)) {
            	foreach ($values as $key => $row) {
                	$this->getConnection()->insertArray($this->getMainTable(), $fields, array($row));
            		
                	$this->getConnection()->insertArray($this->getTable('klk_carrier_tablerate'), array('carrier_id', 'tablerate_id'), array(array('carrier_id' => $carrierId, 'tablerate_id' => $this->getConnection()->lastInsertId($this->getMainTable()))));
            	}
    ...
    public function uploadAndImport(\Magento\Framework\DataObject $object)
    {
         ...
         */
        if (empty($_FILES['groups']['tmp_name'][$object->_carrierCode]['fields']['import']['value'])) {
            return $this;
        }

        $this->carrierCode = $object->_carrierCode;
        $filePath = $_FILES['groups']['tmp_name'][$this->carrierCode]['fields']['import']['value'];
        ...
        try {
            $carrier = $this->carrierFactory->create()->load($this->carrierCode, 'code');
            
            #obtenemos los id de los registros en tables rates que pertenecen a a este courier
            $tablerateIds = array();

            $carrierCollection = $this->carrierCollectionFactory->create()->filterTableRatesByCarrierCode($this->carrierCode)->getData();
            
            foreach($carrierCollection as $tablerate) {
                $tablerateIds[] = $tablerate['tablerate_id'];
            }
            ...
            foreach ($this->import->getData($file, $websiteId, $conditionName, $conditionFullName) as $bunch) {
                $this->importData($columns, $bunch,(int) $carrier->getData('carrier_id'));
            }
        } catch (\Exception $e) {
            $this->logger->critical($e);
            throw new \Magento\Framework\Exception\LocalizedException(
